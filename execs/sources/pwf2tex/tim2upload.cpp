...
bool tim2download(const void *tim2) {
  ...
  for (i = 0; i < hdr->textures_count; i += 1) {
    ...
    gs::ReadTexture(tex0.psm, tex0.tb_addr, tex0.tb_width, 0, 0, segment->width,
                    segment->height, pixels);
  }
...
bool tim2upload(const void *tim2) {
  ...
  for (i = 0; i < hdr->textures_count; i += 1) {
    ...
    gs::WriteTexture(tex0.psm, tex0.tb_addr, tex0.tb_width, 0, 0,
                     segment->width, segment->height, pixels);
  }
