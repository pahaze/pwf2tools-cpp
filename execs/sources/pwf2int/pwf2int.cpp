...
cmd_t commands[] = {
    ...
    {"create",
     ...
     cmd_create},
    {"optimize",
     "[folder] [out-neworderfile]",
     "Attempts to optimize the _order.txt file of a folder to achieve lzss "
     "compression. Rich's note: I DO NOT RECOMMEND USING THIS.",
     {"optimise"},
     1,
     cmd_optimize}};

#define assertheader(i, s, ...)                           \
  if (false == ptr2int::checkheader((const void *)(i))) { \
    fprintf(stderr, s, __VA_ARGS__);                      \
    return 1;                                             \
  }
...
intsection_offset_table_t get_int_section_offsets(
    std::vector<intfile_t> &intfiles, int lzss_size) {
  ...
  r.characters =
      r.filename_table + (sizeof(ptr2int::filename_entry_t) * intfiles.size());
  size_t fnsize = 0;
...
void build_int_section(int restype, std::vector<intfile_t> &intfiles,
                       const byte *lzss, int lzss_size, byte *out) {
  intsection_offset_table_t r = get_int_section_offsets(
      intfiles, lzss_size);  // get offsets for the sections
  ptr2int::header_t *hdr =
      (ptr2int::header_t *)(out + r.hdr);   // establish a header
  u32 *offsets = (u32 *)(out + r.offsets);  // pointer of u32s for offsets
  ptr2int::filename_entry_t *fentries =
      (ptr2int::filename_entry_t
           *)(out + r.filename_table);  // pointer of filenames entries
  char *characters = (char *)(out + r.characters);  // what
  ptr2int::lzss_header_t *lzss_sec =
      (ptr2int::lzss_header_t
           *)(out + r.lzss);  // get the lzss header for the section
  ...
  for (u32 i = 0; i < intfiles.size(); i += 1) {  // for each file..
    ...
    size_t fnlen = intfile.filename.length() +
                   1;  // set length of filename + 1 null terminator
    const char *cstr = intfile.filename.c_str();  // get cstring of filename
    memcpy(characters + fnoffs, cstr,
           fnlen);  // copy the cstring to characters + file
...
static int cmd_list(int argc, char *args[]) {
  ...
  while (hdr->resourcetype != INT_RESOURCE_END) {  // check all sections
    ptr2int::filename_entry_t *entries =
        ptr2int::getfilenameentries(hdr);           // get the filenames
    char *characters = ptr2int::getfilenames(hdr);  // get some more
    for (u32 i = 0; i < hdr->filecount; i += 1) {   // for each file
      printf("%s - %d bytes\n", characters + entries[i].offset,
             entries[i].sizeof_file);  // list name - size
    }
...
static int cmd_extract(int argc, char *args[]) {
  ...
  while (hdr->resourcetype != INT_RESOURCE_END) {
    const char *restypename =
        ptr2int::typenames[hdr->resourcetype];  // get the type
    printf("INFLATE: Section %s\n", restypename);
    ptr2int::lzss_header_t *lzss =
        ptr2int::getlzssheader(hdr);  // get lzzs header
    byte *uncompressed = (byte *)(malloc(
        lzss->uncompressed_size));  // get the uncompressed size from header
    byte *compressed = lzss->data;  // get the data
    memset(history, 0, 4096);       // set history to 0s
    lzss_decompress(
        12, 4, 2, 2, history, compressed, lzss->compressed_size, uncompressed,
        lzss->uncompressed_size);  // uncompress into uncompressed size
    ptr2int::filename_entry_t *entries =
        ptr2int::getfilenameentries(hdr);                 // get names
    ...
    printf("Extracting %d files (TYPE: %s)\n", hdr->filecount,
           restypename);  // extractin # files (type: resource type)

    snprintf(lbuf, sizeof(lbuf), "%s/%s", dirname,
             restypename);  // set lbuf to dirname/resource type
    makedir(lbuf);          // make lbuf if it doesnt exist

    snprintf(lbuf, sizeof(lbuf), "%s/%s/_order.txt", dirname,
             restypename);               // set lbuf to include _order
    ...
    for (u32 i = 0; i < hdr->filecount; i += 1) {     // iterate over every file
      ...
      const char *filename =
          fnchars + entry.offset;  // get filename from the offset
      snprintf(lbuf, sizeof(lbuf), "%s/%s/%s", dirname, restypename,
               filename);  // set buf to dir/resource/filename

      printf("WRITE: %s (%d bytes)\n", filename,
             entry.sizeof_file);          // write: filename (file size)
      FILE *outfile = fopen(lbuf, "wb");  // open out file in writebyte
      if (NULL == outfile) {              // if it cant, move on
        INFO("Couldn't open %s...\n", lbuf);
        continue;
      }
      fwrite(uncompressed + fileoffsets[i], 1, entry.sizeof_file,
             outfile);  // write the uncompressed + fileoffsets from memory into
                        // outfile
...
static int cmd_create(int argc, char *args[]) {
  ...
  for (int i = 1; i < INT_RESOURCE_TYPE_COUNT;
       i += 1) {  // resource type count is 8
    const char *restypename =
        ptr2int::typenames[i];  // what name do we use? pick from the array
    snprintf(lbuf, sizeof(lbuf), "%s/%s", dirname,
             restypename);  // make lbuf equal dir/type
    ...
    if (!direxists(lbuf)) {  // if it dont exist, move on
      ...
      while ((filename = iter->next()) != NULL) {
        snprintf(lbuf, sizeof(lbuf), "%s/%s/%s", dirname, restypename,
                 filename);  // set lbuf to dirname/resource type/filename
        printf("CHECK: %s\n", lbuf);  // does it exist?
        ...
        folderlen =
            ALIGN(folderlen + len, 0x10);  // align folderlen and len with 0x10
        folderdata =
            (byte *)(realloc(folderdata, folderlen));  // extend folder data
        fread(folderdata + fdp, 1, len,
              f);  // export file into folderdata+fdp bytes
        pad_folderdata(folderdata, fdp + len, folderlen);  // 0fill folderdata
        fclose(f);                                         // close the file
        intfiles.push_back(intfile_t(std::string(filename), len,
                                     fdp));  // add the new intfile to vector
      ...
      int complen =
          lzss_compress(12, 4, 2, 2, history, folderdata, folderlen,
                        NULL);  // compress the folderddata, get the length

      printf("COMPRESS: Output %d bytes\n", complen);  // output compressed
                                                       // bytes
      memset(history, 0, 4096 * 2);                 // fill history with 0 again
      byte *lzss_data = (byte *)(malloc(complen));  // allocate complen to bytes
      lzss_compress(12, 4, 2, 2, history, folderdata, folderlen,
                    lzss_data);  // compress again, but now output lzzs data
      free(folderdata);          // free the data

      printf("Building %s section\n", restypename);
      int seclen = calc_int_section_size(
          intfiles, complen);  // calculate the size for the int ssection
  ...
  printf("Finalizing\n");
  fwrite(&ptr2int::nullhdr, sizeof(ptr2int::nullhdr), 1,
         outfile);  // write nullhdr to file?
...
static int cmd_optimize(int argc, char *args[]) {
  ...
  char lbuf[256];
  byte *history =
      (byte *)(malloc(4096 * 2));   // allocate 4096*2 for keeping history
  std::vector<intfile_t> intfiles;  // vector of int files
  std::vector<int> indices;         // vector of ?????

  FILE *outfile = fopen(outfilename, "a");
  if (outfile == NULL) {  // if the out file couldnt be opened (already there
                          // and access denied)
    ERROR("Couldn't open target file %s\n", outfilename);
    return 1;
  }
  fclose(outfile);
  // seemingly does a quick check to see if it can open it and then just closes
  // it right after??????
  ...
  while ((filename = iter.next()) != NULL) {
    snprintf(lbuf, sizeof(lbuf), "%s/%s", foldername,
             filename);  // set buffer to foldername/filename,
    ...
    int fdp = orig_folderlen;  // folder length is whats read i believe, which
                               // currently is 0
    ...
    */
    orig_folderdata = (byte *)(realloc(
        orig_folderdata, orig_folderlen));  // reallocate folderdata to now fit
                                            // folderlength so we dont overflow
    fread(orig_folderdata + fdp, 1, len,
          f);  // read amount of data and put into folderdata + folderlength
    pad_folderdata(orig_folderdata, fdp + len,
                   orig_folderlen);  // zero-fill the data
    fclose(f);
    indices.push_back(i);  // add i to indicies
    intfiles.push_back(intfile_t(std::string(filename), len,
                                 fdp));  // add an intfile to the vector
  ...
  printf("%d files using %d bytes\n", u32(intfiles.size()),
         orig_folderlen);  //# of int files using folder length
  printf("Compressing...\n");
  memset(history, 0, 4096 * 2);  // fill memory with 0s?
  int orig_complen =
      lzss_compress(12, 4, 2, 2, history, orig_folderdata, orig_folderlen,
                    NULL);      // get the compression length
  ...
  for (;;) {                                 // inf loop
    ...
    for (i = 0; i < indices.size(); i += 1) {
      ...
      memcpy(folderdata + fdp, orig_folderdata + intfile.offset,
             intfile.filesize);
    ...
    int newcomplen =
        lzss_compress(12, 4, 2, 2, history, folderdata, folderlen, NULL);
    if (newcomplen < ccomplen) {
      printf("   Saved %d bytes (%d from original)\n", (ccomplen - newcomplen),
             (newcomplen - orig_complen));
      ccomplen = newcomplen;
...
int main(int argc, char *args[]) {
  if (ptr2int::checkinstall() ==
      false) {  // if it didnt compile well, will barely show
